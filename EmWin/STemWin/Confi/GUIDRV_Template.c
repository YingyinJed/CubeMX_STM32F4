...
uint32_t UCGUI_LCD_CMD =  0X6C00007E;
uint32_t UCGUI_LCD_DATA = 0X6C000080;
/*********************************************************************
*
*       Defines
*
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      PixelIndex = LCD_ReadPoint(x,y);//调用ILI93xx.c文件中的读点函数
    }
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  LCD_Fill(x0,y0,x1,y1,LCD_COLORINDEX);//调用ILI93xx.c文件中的填充函数
}
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  LCD_PIXELINDEX pixel;
	LCD_SetCursor(x,y);
	*(__IO uint16_t *)(UCGUI_LCD_CMD) = lcddev.wramcmd;//写入颜色的值
	for(;xsize > 0 ;xsize--,x++,p++)
	{
		pixel = *p;
